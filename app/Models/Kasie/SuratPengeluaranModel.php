    ...
    public function totalPengandangan($nama_terminal, $now)
    {
        if ($nama_terminal == "Rawa Buaya") {
            return $this->db->table($this->table)
                ...
                ->where(["tanggal_pengeluaran" => $now])
                ->countAllResults();
        } elseif ($nama_terminal == "Pulo Gadung") {
            return $this->db->table($this->table)
                ->select($this->fieldTable)
                ->where(["nama_terminal " => $nama_terminal])
                ->join('ukpd', 'ukpd.id = ukpd_id')
                ...
                ->where(["tanggal_pengeluaran" => $now])
                ->countAllResults();
        } elseif ($nama_terminal == "Pulo Gebang") {
            return $this->db->table($this->table)
                ->select($this->fieldTable)
                ->where(["nama_terminal " => $nama_terminal])
                ->join('ukpd', 'ukpd.id = ukpd_id')
                ...
                ->where(["tanggal_pengeluaran" => $now])
                ->countAllResults();
        } elseif ($nama_terminal == "Tanah Merdeka") {
            return $this->db->table($this->table)
                ->select($this->fieldTable)
                ->where(["nama_terminal " => $nama_terminal])
                ->join('ukpd', 'ukpd.id = ukpd_id')
