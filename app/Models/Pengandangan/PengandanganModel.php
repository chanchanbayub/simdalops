...
class PengandanganModel extends Model
{
    ...
    public function getDataKendaraanPengandangan($now = null)
    {
        if ($now != null) {
    ...
    public function getKendaraanId($id)
    {
        return $this->table($this->table)
            ...
            ->join('bap', 'bap.id = laporan_penindakan.bap_id')
            ->join('status_bap', 'status_bap.id = bap.status_id')
            ->where(["pengandangan.id" => $id])
            ->orderBy('pengandangan.id desc')
            ->get()->getRowArray();
    }

    public function search($nopol)
    {
        $this->table($this->table)
            ->select($this->fieldTable)
            ->join('laporan_penindakan', 'laporan_penindakan.id = pengandangan.laporan_penindakan_id')
    ...
    public function getKendaraanKeluar($keyword = null)
    {
        if ($keyword == null) {
            $this->table($this->table)
                ->select($this->fieldTable)
                ...
                ->join('bap', 'bap.id = laporan_penindakan.bap_id')
                ->join('status_bap', 'status_bap.id = bap.status_id')
                ->where(["bap.status_id" => 6])
                ->where(["status_kendaraan" => "Masuk"])
                ->orderBy('pengandangan.id desc');
        } else {
            $this->table($this->table)
                ->select($this->fieldTable)
